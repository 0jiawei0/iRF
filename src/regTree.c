...
void findBestSplit(double *x, int *jdex, double *y, int mdim, int nsample,
		   ...
		   double *ubest, int *ndendl, int *jstat, int mtry,
                   double *selprob, int *subsetvar, int mcard,
    ...
    *decsplit = 0.0;
    ...
    if ((i+1) <= mcard){
		...
		xrand = unif_rand();
		selectsplit(xrand, tselprob, totprob, last, &jold, mdim);
		j = jold;
	}
	kv = mind[j];
...
void selectsplit(double xrand, double *tselprob, double totprob, int last, int *j, int mdim){
	...
	cumselprob[0] = tselprob[0];
	for (i=1; i < last; ++i){
		cumselprob[i] = cumselprob[i-1]+tselprob[i];
	}
